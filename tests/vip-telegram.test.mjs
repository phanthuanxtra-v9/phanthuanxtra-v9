...
test('VIP Telegram document download enforces 15 MB limit from content-length', async()=>{
  ...
  try{
    await assert.rejects(
      downloadTelegramFile({TELEGRAM_VIP_BOT_TOKEN:'test-token'},'file-1'),
      /exceeds 15 MB limit/
    );
...
test('VIP admin webhook endpoint advertises POST on method mismatch',async()=>{
  const response=await handleVipTelegram(
    new Request('https://example.com/api/admin/telegram/vip-webhook',{method:'GET'}),
    {}
  );
