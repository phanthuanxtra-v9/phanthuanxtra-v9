...
public class MainActivity extends Activity {
  ...
  void processGalleryImage(Uri u){UploadPayload upload=null;try{upload=prepareUpload(u);String up;try(InputStream in=upload.open()){up=api.requestCheckedStream("POST","/media",in,upload.length,upload.contentType);}String image=new JSONObject(up).optString("url","");if(image.isEmpty())throw new Exception("Media API không trả URL");String id=selectedId();if(id==null)return;addGalleryUrl(id,image);}catch(Exception e){ui(()->output.setText("Lỗi upload gallery: "+e.getMessage()));}finally{if(upload!=null)upload.delete();}}
