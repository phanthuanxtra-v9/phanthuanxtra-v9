...
public class MainActivity extends Activity {
  ...
  void ui(Runnable task){
    ...
    mainHandler.post(()->{if(!isFinishing()&&!isDestroyed())task.run();});
  }

  void call(String method,String path,String json,byte[] raw,String type){
    runAsync(()->{try{String r=api.requestChecked(method,path,json,raw,type==null?"application/json":type);ui(()->output.setText(r));}catch(Exception e){ui(()->output.setText("Lỗi: "+e.getMessage()));}});
  }
  void callPublic(String method,String path){
    runAsync(()->{try{String r=api.requestChecked(method,path,null,null,"application/json");ui(()->output.setText(r));}catch(Exception e){ui(()->output.setText("Lỗi kết nối: "+e.getMessage()));}});
  }
  ...
  void toggleFeatured(){String id=selectedId();if(id==null)return;runAsync(()->{try{String r=api.requestChecked("GET","/cars/"+Uri.encode(id),null,null,"application/json");JSONObject c=new JSONObject(r).optJSONObject("car");if(c==null)throw new Exception("Không đọc được xe");boolean next=!c.optBoolean("featured",false);JSONObject body=carPayload(c);body.put("featured",next);String saved=api.requestChecked("PUT","/cars/"+Uri.encode(id),body.toString(),null,"application/json");ui(()->output.setText(saved));}catch(Exception e){ui(()->output.setText("Lỗi nổi bật: "+e.getMessage()));}});}
  void updateCarField(String id,String key,String value){runAsync(()->{try{String r=api.requestChecked("GET","/cars/"+Uri.encode(id),null,null,"application/json");JSONObject c=new JSONObject(r).optJSONObject("car");if(c==null)throw new Exception("Không đọc được xe");JSONObject body=carPayload(c);body.put(key,value);String saved=api.requestChecked("PUT","/cars/"+Uri.encode(id),body.toString(),null,"application/json");ui(()->output.setText(saved));}catch(Exception e){ui(()->output.setText("Lỗi cập nhật: "+e.getMessage()));}});}
