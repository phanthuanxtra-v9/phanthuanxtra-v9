...
function isTrusted(url) {
  return /^https:\/\/phanthuanxtra\.com\/media\/vehicles\/publish-[a-z0-9._-]+\.jpg$/i.test(clean(url));
}

async function fetchImage(url) {
  if (!/^https?:\/\//i.test(url)) throw new Error("Ảnh Admin phải là URL HTTP(S) để đưa qua AI.");
  const u = new URL(url);
  if (!/^https?:$/.test(u.protocol)) throw new Error("URL ảnh không hợp lệ.");
  ...
  if (declared > MAX_BYTES) throw new Error("Ảnh Admin vượt quá 12MB.");
  const bytes = await response.arrayBuffer();
  if (!bytes.byteLength || bytes.byteLength > MAX_BYTES) throw new Error("Ảnh Admin vượt quá 12MB.");
...
async function brandOne(env, url, index, carId) {
  ...
  if (!hasValidPlateBox(ai?.plate_bbox)) {
    throw new Error(`AI không xác định chắc chắn biển số ở ảnh Admin #${index + 1}; publish bị chặn.`);
  }
...
export async function handleAdminVehiclePipeline(request, env) {
  ...
  if (!body || !Array.isArray(body.images) || body.images.length === 0) {
    return json({ error: "Admin đăng xe bắt buộc phải có ít nhất 1 ảnh để AI kiểm tra và branding PT Xtra." }, 400);
  }
