...
function buildCaption(car){
  const title=[clean(car.brand),clean(car.model)].filter(Boolean).join(" ")||clean(car.id);let out=`🔥 <b>${esc(title)}</b>\n\n`;
