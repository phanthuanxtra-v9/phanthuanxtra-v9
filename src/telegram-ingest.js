...
function reportText(ai,inboxId,mediaKey,published=false){
  ...
  const lines=[published?"🚀 ĐÃ PHÂN TÍCH + ĐÃ THAY BIỂN SỐ PT XTRA + TỰ ĐĂNG XE":"✅ ĐÃ NHẬN ẢNH + THÔNG TIN XE",`📦 Inbox ID: ${inboxId}`,`🚗 Xe: ${label}`,a.year?`📅 Năm: ${a.year}`:null,a.mileage!=null?`🛣 ODO: ${a.mileage}`:null,a.price!=null?`💰 Giá: ${a.price}`:null,a.fuel?`⛽ Nhiên liệu: ${a.fuel}`:null,a.color?`🎨 Màu: ${a.color}`:null,`🎯 Độ tin cậy AI: ${Math.round(Number(a.confidence||0)*100)}%`,hasValidPlateBox(a?.plate_bbox)?"🪪 Biển số: Đã xác định vùng và thay bằng PT Xtra":"🪪 Biển số: Chưa xác định chắc chắn, KHÔNG đăng tự động",Array.isArray(a.missing_fields)&&a.missing_fields.length?`⚠️ Cần bổ sung: ${a.missing_fields.join(", ")}`:"✅ Không phát hiện trường bắt buộc còn thiếu","",published?"📌 Trạng thái: Bản ảnh publish đã được tạo trong R2 trước khi tạo bản ghi website.":"📌 Trạng thái: Đã lưu ảnh gốc + bản nháp AI; chưa publish.",published?"🛡 Duplicate protection: telegram_posts.car_id":"⏳ Chưa tự đăng vì chưa đạt ngưỡng AI hoặc chưa xử lý chắc chắn biển số.",mediaKey?`🖼 Ảnh publish: /media/${mediaKey}`:null].filter(Boolean);return lines.join("\n");}
